...
rebuilt_population <- create_pop_series(mye_coc = mye_2011_24, 
  ...
  filter(cohort <= 2019 & year >= 2019 & age %in% 0:5, ) # we only have flows for cohorts 2019 and before
...
rebuilt_population_2020_plus <- create_pop_series(mye_coc = mye_2011_24,
...
adjusted_population_0_15 <- full_new_adjusted_estimates %>%
  ...
  filter(year >= 2019) # agh, two filter calls. fix that. 
